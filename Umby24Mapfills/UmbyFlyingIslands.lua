			...
			for iy = 0, Chunk_Size-1 do
				...
				for iz = Height_0, Height_1 do
					if iz == Height_1 then
                        ...
                        Map.setblock(-1, Map_ID,Offset_X+ix, Offset_Y+iy, iz, 2, 0, 0, 0, 0)
                        
					else
						--World_Block_Set(World_ID, Offset_X+ix, Offset_Y+iy, iz, 3, 0, 0, 0, 0, 0, 0, 0)
                        Map.setblock(-1, Map_ID,Offset_X+ix, Offset_Y+iy, iz, 3, 0, 0, 0, 0)
					end
				end
			end
		end
			...
			for iy = 0, Chunk_Size-1 do
				...
				for iz = Height_0, Height_1 do
					if iz == Height_1 and math.random(40) == 1 then
                        ...
                        --Map.setblock(-1, Map_ID,Offset_X+ix, Offset_Y+iy, iz+1, 17, 0, 0, 0, 0)
					end
				end
			end
		end
		...
		for ix = 0, Chunk_Size-1 do
			for iy = 0, Chunk_Size-1 do
				...
				for iz = Height_0, Height_1 do
					if iz == Height_1 then
						--World_Block_Set(World_ID, Offset_X+ix, Offset_Y+iy, iz, 2, 0, 0, 0, 0, 0, 1, 0)
                        Map.setblock(-1, Map_ID,Offset_X+ix, Offset_Y+iy, iz / Division_Factor, 2, 0, 0, 0, 0)
                        
					else
						--World_Block_Set(World_ID, Offset_X+ix, Offset_Y+iy, iz, 3, 0, 0, 0, 0, 0, 0, 0)
                        Map.setblock(-1, Map_ID,Offset_X+ix, Offset_Y+iy, iz / Division_Factor, 3, 0, 0, 0, 0)
					end
				end
			end
		end
...
function Mapfill_umbyflying(Map_ID,MapX,MapY,MapZ,args)
   ...
   local randomSeed = -144 --math.random(-500,500) / 2 
	local randomSeed2 = math.random(-500,500) / 2
	local newseed = math.random(-500,500) / 2
	
	System.msgAll(Map_ID, "&2Generation Seed: &a" .. tostring(randomSeed))
	System.msgAll(Map_ID, "&2Generation Seed: &a" .. tostring(randomSeed2))
	
	for ix = 0, MapX do -- Create water
		for iy = 0, MapY do
			for iz = 0, 2 do
				Map.setblock(-1, Map_ID, ix, iy, iz, 9, 0, 0, 0, 0)
			end
		end
	end
...
System.msgAll(-1, "&9Umby flying islands reloaded!")
