function Mapfill_normal(Map_ID, Map_Size_X, Map_Size_Y, Map_Size_Z, Argumentstring)
	...
	-- H�hen: (Maximale H�he des Materials)
		...
		for iy = 0, Size do
			...
			if Height > Height_Grass then -- Snow and more
				for iz = 0, Height-1 do
					if iz > Height_Mountain-1 then
						Map.setblock(-1, Map_ID, ix, iy, iz, 36, 0, 0, 0, 0)
					else
						Map.setblock(-1, Map_ID, ix, iy, iz, 1, 0, 0, 0, 0)
					end
				end
			elseif Height > Height_Water then -- Grass
				for iz = 0, Height-1 do
					if iz == Height-1 then
						Map.setblock(-1, Map_ID, ix, iy, iz, 2, 0, 0, 0, 0)
					elseif iz > Height_Stone then
						Map.setblock(-1, Map_ID, ix, iy, iz, 3, 0, 0, 0, 0)
					else
						Map.setblock(-1, Map_ID, ix, iy, iz, 1, 0, 0, 0, 0)
					end
				end
			elseif Height <= Height_Water then -- Water and under
				for iz = 0, Height_Water-1 do
					if iz > Height-1 then
						Map.setblock(-1, Map_ID, ix, iy, iz, 8, 0, 0, 0, 0)
					elseif iz == Height_Water-1 then
						Map.setblock(-1, Map_ID, ix, iy, iz, 12, 0, 0, 0, 0)
					elseif iz > Height_Stone then
						Map.setblock(-1, Map_ID, ix, iy, iz, 3, 0, 0, 0, 0)
					else
						Map.setblock(-1, Map_ID, ix, iy, iz, 1, 0, 0, 0, 0)
