...
function Mapfill_city2(Map_ID, Map_Size_X, Map_Size_Y, Map_Size_Z)
	...
	for x = 0, Map_Size_X-1 do -- fills the map with stone, with an obsidian top layer.
		for y = 0, Map_Size_Y-1 do
			for z=0, Water_Height-1 do
				...
				Map.setblock(-1, Map_ID, x, y, z, mat, 0, 0, 0, 0)
			end
		end
	end
	...
	local time = os.clock()
	local HMap_Scale = 16
	System.msgAll(Map_ID, "&3Building size: &b".. HMap_Scale)
	...
	for x=0, HMap_Size_X do --generate all sizes
		for y = 0, HMap_Size_Y do
			...
			local size_x = math.random(math.floor(HMap_Scale/3),HMap_Scale-4)
			local size_y = math.random(math.floor(HMap_Scale/3),HMap_Scale-4)
				...
				do
					...
					for ix=0, size_x+p_x do
						...
						for iy=1, p_y do
							...
							local oldmat = Map.getblock(Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+size_y+iy, Water_Height)

							if oldmat == 0 then

								Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+size_y+iy, Water_Height, 44, 0, 0, 0, 0)
					...
					for ix=1, p_x do
						...
						for iy=0, size_y+p_y do
							...
							local oldmat = Map.getblock(Map_ID, (x*HMap_Scale)+size_x+ix, (y*HMap_Scale)+iy, Water_Height)

							if oldmat == 0 then

								Map.setblock(-1, Map_ID, (x*HMap_Scale)+size_x+ix, (y*HMap_Scale)+iy, Water_Height, 44, 0, 0, 0, 0)
				...
				for ix=0, size_x do
					...
					for iy=0, size_y do
						...
						for iz=0, height do
							...
							local oldmat = Map.getblock(Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, z)
							...
							if (ix == size_x or iy == size_y or ix == 0 or iy == 0) then
								...
								if (ix == 0 and iy == 0) or (ix == size_x and iy == size_y) or
								   ...
								   (ix == 0 and iy == size_y) or (ix == size_x and iy == 0) then
									...
									Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, z, material, 0, 0, 0, 0)

								elseif oldmat ~= 0 then

									Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, z, 0, 0, 0, 0, 0)

								else

									Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, z, mat, 0, 0, 0, 0)
						...
						Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, Water_Height, material, 0, 0, 0, 0) --stone ground..

						--Map.setblock(-1, Map_ID, (x*HMap_Scale)+ix, (y*HMap_Scale)+iy, Water_Height+oheight, 21, 0, 0, 0, 0)

					end

				end

				--Map.setblock(-1, Map_ID, (x*HMap_Scale)+1, (y*HMap_Scale)+1, Water_Height+height+1, 227, 0, 0, 1, 0) --fan
	...
	System.msgAll(Map_ID, "&2Constructed buildings in &a"..os.clock()-time.."&2s.")
	System.msgAll(Map_ID, "&2Took a total of &a"..os.clock()-ttime.."&2s.")
end

System.msgAll(-1, "&2Citygen Reloaded.")