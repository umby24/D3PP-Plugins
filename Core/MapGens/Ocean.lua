function Mapfill_ocean(Map_ID, Map_Size_X, Map_Size_Y, Map_Size_Z)
	...
	System.msgAll(Map_ID, "&c Iterations:"..tostring(Iterations))
	...
	local Height_Dunes = Map_Size_Z * 0.3
	local Height_Sand = Map_Size_Z * 0.10
	local Height_Water = Map_Size_Z * 0.5 - 1
	local Height_Dirt = Map_Size_Z * 0.1
	...
	--System.msgAll(Map_ID, "&c test")
	
	for ix = 0, Size do
		for iy = 0, Size do
			Counter[ix][iy] = math.random(Height_Dunes)
			if Counter[ix][iy] < Height_Sand then
				Counter[ix][iy] = (Counter[ix][iy] + Height_Sand*10) / 11
			end
		end
	end
	
	--System.msgAll(Map_ID, "&c test1")
			...
			if Height >= Height_Water then
				for iz = 0, Height do
					...
					elseif iz > Height_Dirt then
						Map.setblock(-1, Map_ID, X, Y, Z, 3, 0, 0, 0, 0)
					else
						Map.setblock(-1, Map_ID, X, Y, Z, 1, 0, 0, 0, 0)
					end
				end
			else
				for iz = 0, Height_Water do
					Z = iz
					if iz > Height then
						Map.setblock(-1, Map_ID, X, Y, Z, 8, 0, 0, 0, 0)
