...
function Terrabyte_Parse(String, Name, Default)
	--System.msgAll(-1, "&c> Debug")
	String_2_Find = Name.."="
	
	--System.msgAll(-1, "&c> "..String_2_Find)
	
	Start, End = string.find(String, String_2_Find)
	
	--System.msgAll(-1, "&c> "..tostring(Start).." -> "..tostring(End))
	...
	if Start and End then
		...
		--System.msgAll(-1, "&c>Parsed: "..string.sub(String, End+1, Space_Start))
		return string.sub(String, End+1, Space_Start)
	else
		--System.msgAll(-1, "&c>Default: "..Default)
		return Default
	end
end
...
-- #########################################################################################################
	...
	local Climate_Chunk_Size = 2^Climate_Iterations
...
-- #########################################################################################################
	...
	local Mountain_Chunk_Size = 2^Mountain_Iterations
	...
	for ix = 0, Mountain_Iteration_Size_X do
		for iy = 0, Mountain_Iteration_Size_Y do
			...
			Mountain_Heightmap[ix][iy] = Mountain_Heightmap[ix][iy]*(5^ -Climate_Factor) - Climate_Factor*Map_Size_Z*0.2
		end
	end
...
-- #########################################################################################################
	...
	local Normal_Chunk_Size = 2^Normal_Iterations
...
-- #########################################################################################################
	...
	for ix = 0, Mountain_Iteration_Size_X, Fill_Step do
		for iy = 0, Mountain_Iteration_Size_Y, Fill_Step do
			...
			for iz = 0, Z do
				Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Stone, 0, 0, 0, 0)
			end
		end
	end
	...
	for ix = 0, Normal_Iteration_Size_X, Fill_Step do
		for iy = 0, Normal_Iteration_Size_Y, Fill_Step do
			if Map_Normal_Z_Offset+Normal_Heightmap[ix][iy] > Map_Mountain_Z_Offset+Mountain_Heightmap[ix][iy] then
				...
				for iz = Z, 0, -1 do
					if Map.getblock(Map_ID, ix, iy, iz) == 0 then
						if iz == Z then -- If it's on top
							if Climate_Map[ix][iy] <= 2+SinCos_Value then -- If it's not near the desert
								if iz <= Map_Ocean_Z_Offset-2 then -- Undea ocean
									Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Gravel, 0, 0, 0, 0)
								elseif iz <= Map_Ocean_Z_Offset+1 then -- At ocean
									Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Sand, 0, 0, 0, 0)
								else -- Above ocean
									Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Grass, 0, 0, 0, 0)
								end
							else -- It's near the desert
								Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Dirt, 0, 0, 0, 0)
							end
	...
	for ix = 0, Normal_Iteration_Size_X, Fill_Step do
		for iy = 0, Normal_Iteration_Size_Y, Fill_Step do
			for iz = Map_Ocean_Z_Offset, 0, -1 do
				if Map.getblock(Map_ID, ix, iy, iz) == 0 then
					Map.setblock(-1, Map_ID, ix, iy, iz, Mat_Water, 0, 0, 0, 0)
				else
					break
				end
			end
...
-- #########################################################################################################
	...
	System.msgAll(Map_ID, "&c Values: ci="..tostring(Climate_Iterations).." mi="..tostring(Mountain_Iterations).." mr="..tostring(Map_Mountain_Roughness_Constant).." ni="..tostring(Normal_Iterations).." nr="..tostring(Map_Normal_Roughness_Constant))
	System.msgAll(Map_ID, "&c Map created in "..string.sub(tostring(os.clock()-Time_Start), 1, 4).."s. (Terrabyte by Dadido3)")
